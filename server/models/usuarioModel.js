...
export const updateUsuarioById = (data, id, result) => {
    db.query("UPDATE usuario SET nomeUsuario = ?, emailUsuario = ?, cpfUsuario = ?, endUsuario = ?, senhaUsuario = ?, tipoUsuario = ?, telefoneUsuario = ?  WHERE idUsuario = ?", 
        [data.nomeUsuario, data.emailUsuario, data.cpfUsuario, data.endUsuario, data.senhaUsuario, data.tipoUsuario, data.telefoneUsuario, id], (err, results) => {             
        if(err) {
            console.log(err);
            result(err, null);
        } else {
